...
/*14. Возвращаемся обратно к созданному роуту и наполним его логикой. В начале как правило обращаемся к коллекции movies, для добавления нового элемента 
используем метод insertOne, внутрь которого передаем тело запроса или req.body и по сути это все. Теперь в then в случае успешного добавления данных мы 
возвращаем информацию о результате. Сам ответ будет возвращаться со статусом 201, который обозначает успешное добавление нового элемента. А в блоке catch, 
как и в предыдущих роутах используем функцию handleError, в которую передаем строку с ошибкой. То есть, если что-то пойдет не так, мы об этом узнаем. Идем в 
Постман, изменяем тип отправляемого запроса на post, далее указываем нужный роут. Теперь в панели полей запроса выбираем формат данных JSON. Берем документ
и передаем его. Нажимаем Send и если все сделано верно, то получаем объект с инфой об успешном добавлении документа в БД. Теперь наш API может добавлять 
...
/* 15. И в заключении нам нужно научить его их обновлять. Для этого тоже создаем роут. Используем для обновления метод, который называется patch. Механизм 
обновления в общем виде будет выглядеть следующим образом. Для начала в базе нам нужно будет найти требуемый элемент. Делать мы это будем по id, для этого 
указываем в роуте динамический id. Обновленные данные точно также как и при создании элемента мы будем пробрасывать через тело запроса. После чего обновлять
 ими найденный элемент. По сути описанная логика чем-то похожа на все предыдущие методы. За основу берем удаление элемента из БД. Точно также в самом начале
 передаваемый в пути id мы будем проверять на валидность. Далее обращаемся к коллекции movies. И используем метод updateOne. А я напомню, что он принимает
 два аргумента, первый это id элемента, по которому найдется документ, который нужно обновить. Второй, это объект со специальным оператором $set, он принимает
 обновленные данные, которые как и говорилось мы будем брать из тела запроса. И это все. Остальная логика остается неизменной. В then мы возвращаем инфу об
 успешном обновлении. В catch ошибку, если что-то пойдет не так. Ну а в блок else мы попадем, если переданный id не будет валидным. И теперь опять возвращаемся
 в Постман. Копируем id фильма, который планируем обновлять, добавляем его к пути. Далее изменяем метод запроса на patch. После чего, нам лишь осталось 
 подкорректировать данные, которыми мы будем обновлять документ. Теперь нажимаем Send, и если все сделано верно, то в ответе, который пришел из сервера, мы
 получим объект с инфой об успешной операции обновления. Как минимум поле modifiedCount:1 говорит о том, что один элемент в базе был обновлен. Переходим во 
 вкладку запроса всех фильмов и повторим запрос за всеми документами из коллекции и в возвращаемом ответе, видим, что данные были благополучно обновлены.
 Теперь наш API выполняет все необходимые операции и благополучно готов к использованию. */
